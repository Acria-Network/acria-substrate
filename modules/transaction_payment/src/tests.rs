...
const CALL: &<Runtime as frame_system::Config>::Call =
	&Call::Currencies(module_currencies::Call::transfer(BOB, USDA, 12));
...
fn charges_fee() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(ACRI, &ALICE), (100000 - fee).into());
		...
		assert_eq!(
			Currencies::free_balance(ACRI, &ALICE),
...
fn charges_fee_when_pre_dispatch_and_native_currency_is_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert!(ChargeTransactionPayment::<Runtime>::from(0)
			...
			.is_ok());
		assert_eq!(Currencies::free_balance(ACRI, &ALICE), 100000 - fee);
	});
}
...
fn refund_fee_according_to_actual_when_post_dispatch_and_native_currency_is_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(ACRI, &ALICE), 100000 - fee + refund);
	});
}
...
fn charges_fee_when_validate_and_native_is_not_enough() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(<Currencies as MultiCurrency<_>>::transfer(USDA, &ALICE, &BOB, 1000));
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(ACRI, &BOB), 0);
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(USDA, &BOB), 1000);
		...
		// 	false
		// ));
		// assert_eq!(DEXModule::get_liquidity_pool(ACRI, USDA), (10000, 1000));
		...
		// assert_eq!(Currencies::free_balance(ACRI, &BOB), 0);
		// assert_eq!(Currencies::free_balance(USDA, &BOB), 749);
		// assert_eq!(DEXModule::get_liquidity_pool(ACRI, USDA), (10000 - 2000, 1251));
	});
}
...
fn set_default_fee_token_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(TransactionPayment::default_fee_currency_id(&ALICE), Some(USDA));
