...
use acria_primitives::{AccountPublic, Balance, Nonce};
...
pub struct Extensions {
	...
	pub fork_blocks: sc_client_api::ForkBlocks<acria_primitives::Block>,
	/// Known bad block hashes.
	pub bad_blocks: sc_client_api::BadBlocks<acria_primitives::Block>,
}
...
pub fn mainnet_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		...
		// Bootnodes
		vec![
			"/dns/bootnode-1.acria.network/tcp/30333/p2p/12D3KooWFHSc9cUcyNtavUkLg4VBAeBnYNgy713BnovUa9WNY5pp".parse().unwrap(),
			"/dns/bootnode-2.acria.network/tcp/30333/p2p/12D3KooWAQqcXvcvt4eVEgogpDLAdGWgR5bY1drew44We6FfJAYq".parse().unwrap(),
			"/dns/bootnode-3.acria.network/tcp/30333/p2p/12D3KooWCT7rnUmEK7anTp7svwr4GTs6k3XXnSjmgTcNvdzWzgWU".parse().unwrap(),
		],
		// Telemetry
		TelemetryEndpoints::new(vec![(TELEMETRY_URL.into(), 0)]).ok(),
		// Protocol ID
...
fn testnet_genesis(
	...
	const INITIAL_BALANCE: u128 = 100_000_000 * ACRI;
	const INITIAL_STAKING: u128 =   1_000_000 * ACRI;
	...
	GenesisConfig {
		...
		orml_tokens: Some(TokensConfig {
			endowed_accounts: endowed_accounts
				...
				.flat_map(|x| {
					vec![
						(x.clone(), CurrencyId::Token(TokenSymbol::USDA), INITIAL_BALANCE),
					]
				})
				.collect(),
		}),
...
fn mainnet_genesis(
	...
	let balances = initial_authorities
		...
		.chain(endowed_accounts.iter().cloned().map(|x| (x.0.clone(), x.1 * ACRI)))
		.chain(
			get_all_module_accounts()
				.iter()
				.map(|x| (x.clone(), existential_deposit)),
...
/// Token
pub fn acria_properties() -> Properties {
	let mut p = Properties::new();
	p.insert("ss58format".into(), 42.into());
	p.insert("tokenDecimals".into(), 18.into());
	p.insert("tokenSymbol".into(), "ACRI".into());
	p
}
