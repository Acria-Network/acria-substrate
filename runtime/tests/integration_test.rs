	...
	ExtBuilder::default()
		.balances(vec![
			(
				...
				CurrencyId::Token(TokenSymbol::USDA),
				amount(1000),
			),
		])
		.build()
		.execute_with(|| {
			...
			// );
			// assert_eq!(
			// 	Currencies::free_balance(CurrencyId::Token(TokenSymbol::USDA), &AccountId::from(BOB)),
			// 	amount(500)
			// );
            //
			// // delay < SevenDays
...
fn test_evm_module() {
	ExtBuilder::default()
		.balances(vec![
			(alice_account_id(), CurrencyId::Token(TokenSymbol::ACRI), amount(1 * MILLI_ACRI)),
			(bob_account_id(), CurrencyId::Token(TokenSymbol::ACRI), amount(1 * MILLI_ACRI)),
		])
		.build()
		.execute_with(|| {
			assert_eq!(Balances::free_balance(alice_account_id()), amount(1 * MILLI_ACRI));
			assert_eq!(Balances::free_balance(bob_account_id()), amount(1 * MILLI_ACRI));
			...
			assert_ok!(Currencies::transfer(
				...
				CurrencyId::Token(TokenSymbol::ACRI),
				amount(10 * MICRO_ACRI)
			));
			assert_eq!(Balances::free_balance(alice_account_id()), 1009999999999989633000000000000000);
			assert_eq!(Balances::free_balance(bob_account_id()), amount(1 * MILLI_ACRI) - amount(10 * MICRO_ACRI));
		});
}
