...
pub mod currency {
	...
	pub const ACRI: Balance = DOLLARS;
	pub const MILLI_ACRI: Balance = ACRI / 1_000;
	pub const MICRO_ACRI: Balance = ACRI / 1_000_000;
}
...
impl TryFrom<u8> for TokenSymbol {
	...
	fn try_from(v: u8) -> Result<Self, Self::Error> {
		match v {
			0 => Ok(TokenSymbol::ACRI),
			1 => Ok(TokenSymbol::USDA),
			_ => Err(()),
		}
	}
}
